export class Financiamento {
  ...
  CalcularTotais() {
    var Total = {
      ...
      valorTotalFinanciamento: this.formataMascara("BRL", this.valorTotalFinanciamento),
  ...
  financiarPrice() {
    ...
    var parteDeCima = Math.pow(1 + this.interestedRate, this.periods) * this.interestedRate;
    ...
    const valorTotalFinanciamento = this.formataMascara("BRL", ValorPago);
    const valorTotalJuros = this.formataMascara("BRL", jurosTotal);
    return {
      parcelas,
  ...
  financiarSac() {
    ...
    for (let k = 1; k <= this.periods; k++) {
      const interestRateBalance = (this.periods - k + 1) * this.amortization * this.interestedRate;
    ...
    const valorTotalJuros = this.formataMascara("BRL", valuePaidWithInterestedRate);
